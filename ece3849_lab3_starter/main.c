...
extern volatile uint32_t MultiPeriodInterval;
extern volatile uint32_t NumPeriods;
float PeriodCLKCyc;
float AvgPeriod;
...
//signals the frequency task every 100ms
void Freq_Clock_Task(UArg arg1, UArg arg2){
    Semaphore_post(SignalFreq);//signal Freq task
}
void Frequency_Task(UArg arg1, UArg arg2){
    IntMasterEnable();
    initComp();
    while(true){
       Semaphore_pend(SignalFreq, BIOS_WAIT_FOREVER);//waits to be signaled
       IArg key = GateHwi_enter(gateHwi1);//protect global data shared with interrupt
       PeriodCLKCyc = (float)MultiPeriodInterval/(float)NumPeriods;//find clock cycles per period
       AvgPeriod = 1/(PeriodCLKCyc/gSystemClock);//find the frequency
       MultiPeriodInterval = 0;//reset back to 0
       NumPeriods = 0;//reset back to 0
       GateHwi_leave(gateHwi1, key);

    }
}
//retrieves button data from GPIO when signaled
    ...
    tRectangle rectFullScreen = {0, 0, GrContextDpyWidthGet(&sContext)-1, GrContextDpyHeightGet(&sContext)-1};
      while (true) {
        ...
        if(OscilloscopeMode == 0){//if in waveform mode...
            ...
            snprintf(str, sizeof(str), "f = %.2fHz", AvgPeriod); // convert freq to string
            GrStringDraw(&sContext, str, /*length*/ -1, /*x*/ 10, /*y*/ 120, /*opaque*/ false);
            /////////////////////////Draws trigger icon//////////////////////
